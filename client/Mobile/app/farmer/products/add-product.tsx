...
const AddProductScreen = () => {
  ...
  const [freshness, setFreshness]     = useState<Freshness>("Freshly Harvested");
  const [description, setDescription] = useState("");

  const handleImagePick = () => {
    Alert.alert("Product Photo", "Choose how to add a photo", [
      {
  ...
  const handleSave = () => {
    ...
    // ── Push new product into shared store ──
    const newProduct = {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ paddingTop: STATUS_BAR + 16, paddingHorizontal: 20, paddingBottom: 32, backgroundColor: "#166534" }}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <Text style={{ color: "white", fontWeight: "700", fontSize: 20, marginRight: 16 }}>Add Product</Text>
          </View>
        </View>
          ...
          <TouchableOpacity onPress={handleImagePick} style={{ backgroundColor: "#fff", borderRadius: 16, borderWidth: 1, borderColor: "#e5e7eb", alignItems: "center", justifyContent: "center", height: 160, overflow: "hidden", marginBottom: image ? 8 : 20 }}>
            {image ? (
              ...
              <View style={{ alignItems: "center", gap: 8 }}>
                ...
                <Text style={{ fontSize: 13, fontWeight: "700", color: "#6b7280" }}>Tap to add product photo</Text>
